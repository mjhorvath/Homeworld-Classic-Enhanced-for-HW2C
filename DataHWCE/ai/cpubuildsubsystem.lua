...
function CpuBuildSS_SpecialSubSystemDemand()	
	...
	if (s_selfTotalValue > 160 and s_militaryStrength >  - 5 and UnderAttackThreat() <  - 70 and (timeSinceLastSubSysDemand > sg_specialSubSysDelayTime or GetRU() > 3000)) then	
		...
		local productionDemand = SubSystemDemandGet(FIGHTERPRODUCTION) + SubSystemDemandGet(CORVETTEPRODUCTION) + SubSystemDemandGet(FRIGATEPRODUCTION) + SubSystemDemandGet(CAPSHIPPRODUCTION) + SubSystemDemandGet(PLATFORMPRODUCTION) + SubSystemDemandGet(RESEARCH) + SubSystemDemandGet(ADVANCEDRESEARCH)
		aitrace("CpuBuildSS_SpecialSubSystemDemand: productionDemand:" .. productionDemand)
...
-- issue: why add a value equal to zero?
				if (numQueued == 0) then	
					local numberBuilt = NumSubSystems(b) + numQueued
					local demand = 1
-- issue: why multiply by a value equal to one?
		...
		if (productionId ~= 0) then	
			...
			if (ssdemand < 0) then	
				...
				if (sg_retireCountCheck >= 2) then	
					aitrace("CpuBuildSS_RetireVaygrProductionSubSystems: ssdemand:" .. ssdemand .. " sg_retireCountCheck:" .. sg_retireCountCheck)
...
function CpuBuildSS_ProcessEachBuildShip()	
	...
	aitrace("****CpuBuildSS_ProcessEachBuildShip: s_numFiSystems:" .. s_numFiSystems .. " s_numCoSystems:" .. s_numCoSystems .. " s_numFrSystems:" .. s_numFrSystems)
	...
	aitrace("****CpuBuildSS_ProcessEachBuildShip: currentFiDemand:" .. currentFiDemand .. " currentCoDemand:" .. currentCoDemand .. " currentFrDemand:" .. currentFrDemand)
	...
	for i = 0, (bcount - 1) do	
		...
		if (sg_buildBCHyperspace == 1 and BuildShipType(buildShipId) == kBattleCruiser) then	
			...
			if (BuildShipHasSubSystem(buildShipId, HYPERSPACE) == 0 and UnderAttackThreat() <  - 5 and BuildShipCanBuild(buildShipId, HYPERSPACE) == 1) then	
				aitrace("****CpuBuildSS_ProcessEachBuildShip: Build BC Hyperspace")
		...
		if (highestPrioritySSOverall <= maxRealDemand and maxRealDemand >= 0) then	
			...
			if (sscount > 0) then	
				...
				if (sscount == 1 and economicValue > 0) then	
					allowBuild = 1
					aitrace("****CpuBuildSS_ProcessEachBuildShip: sscount:" .. sscount .. " economicValue:" .. economicValue)
				elseif (sscount == 2 and economicValue > 1) then	
					...
					aitrace("****CpuBuildSS_ProcessEachBuildShip: ship:" .. i .. " currentFiDemand:" .. currentFiDemand .. " currentCoDemand:" .. currentCoDemand .. " currentFrDemand:" .. currentFrDemand)
					aitrace("****CpuBuildSS_ProcessEachBuildShip: ship:" .. i .. " maxDemandOnShip:" .. maxDemandOnShip)
				end
			else
				allowBuild = 1
			end
			...
			if (allowBuild == 1) then	
				...
				if (BuildShipCanBuild(buildShipId, bestProdSS) == 1) then	
					aitrace("****CpuBuildSS_ProcessEachBuildShip: BuildSS in new func")
...
function DoSubSystemDemand_Hiigaran()	
	...
	if (sg_dofighters == 0) then
		SubSystemDemandSet(FIGHTERPRODUCTION, -9999)
	end
	-- Demand for RESEARCH
	...
	if (researchcount == 0) then	
		...
		if (sg_docorvettes == 0) then
			SubSystemDemandSet(CORVETTEPRODUCTION, -9999)
		end
		if (sg_dofrigates == 0) then
			SubSystemDemandSet(FRIGATEPRODUCTION, -9999)
		end
