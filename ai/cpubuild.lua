...
function CpuBuild_PersonalityDemand()	
	...
	-- DEFINED ROLES stuff (has no effect since it's called by "CpuBuild_Init")
--	if (sg_dofighters == 0) then
--		sg_classPersonalityDemand[eFighter] = -9999
--	end
--	if (sg_docorvettes == 0) then
--		sg_classPersonalityDemand[eCorvette] = -9999
--	end
--	if (sg_dofrigates == 0) then
--		sg_classPersonalityDemand[eFrigate] = -9999
--	end
	...
	aitrace("CpuBuild_PersonalityDemand: sg_classPersonalityDemand[eFighter]:" .. sg_classPersonalityDemand[eFighter].. " sg_classPersonalityDemand[eCorvette]:" .. sg_classPersonalityDemand[eCorvette].. " sg_classPersonalityDemand[eFrigate]:" .. sg_classPersonalityDemand[eFrigate])
end
...
function DetermineAntiChassisDemand(enemyIndex)	
	...
	local AFrDemand = 0
	-- fighter demand
	local enemy_FiValue = PlayersMilitary_Fighter(enemyIndex, player_max)
	local self_AFiValue = PlayersMilitary_AntiFighter(s_playerIndex, player_max)
	-- DEFINED ROLES stuff
	if (DefinedRolesActivated == 1) then
		enemy_FiValue = PlayersMilitary_Fighter(player_enemy, player_total)
		self_AFiValue = PlayersMilitary_AntiFighter(player_ally, player_total)
	end
	-- was enemy_FiValue*kFighterCounterScale - self_AFiValue = which I don't believe was proper
	local enemy_FiTotal = (enemy_FiValue - self_AFiValue) * kFighterCounterScale
	-- does enemy have more fighters by X% then we have antiFighter power?
	if (enemy_FiTotal < 0) then	
		enemy_FiTotal = 0
	end
	-- corvette demand
	local enemy_CoValue = PlayersMilitary_Corvette(enemyIndex, player_max)
	local self_ACoValue = PlayersMilitary_AntiCorvette(s_playerIndex, player_max)
	-- DEFINED ROLES stuff
	if (DefinedRolesActivated == 1) then
		enemy_CoValue = PlayersMilitary_Corvette(player_enemy, player_total)
		self_ACoValue = PlayersMilitary_AntiCorvette(player_ally, player_total)
	end
	-- was enemy_CoValue*kCorvetteCounterScale - self_ACoValue
	local enemy_CoTotal = (enemy_CoValue - self_ACoValue) * kCorvetteCounterScale
	...
	-- frigate demand
	local enemy_FrValue = PlayersMilitary_Frigate(enemyIndex, player_max) - MSfrig_correction
	local self_AFrValue = PlayersMilitary_AntiFrigate(s_playerIndex, player_max)
	-- DEFINED ROLES stuff
	if (DefinedRolesActivated == 1) then
		enemy_FrValue = PlayersMilitary_Frigate(player_enemy, player_total)
		self_AFrValue = PlayersMilitary_AntiFrigate(player_ally, player_total)
	end
	-- was enemy_FrValue*kFrigateCounterScale - self_AFrValue
	local enemy_FrTotal = (enemy_FrValue - self_AFrValue) * kFrigateCounterScale
	...
	aitrace("DetermineAntiChassisDemand: AFiDemand:" .. AFiDemand .. " ACoDemand:" .. ACoDemand .. " AFrDemand:" .. AFrDemand)
end
...
function DetermineChassisDemand(enemyIndex)	
	...
	-- DEFINED ROLES stuff (maybe redundant)
--	if (sg_dofighters == 0) then
--		FiDemand = -9999
--	end
--	if (sg_docorvettes == 0) then
...
--	end
	...
	aitrace("DetermineChassisDemand: FiDemand:" .. FiDemand .. " CoDemand:" .. CoDemand .. " FrDemand:" .. FrDemand)
end
...
--	end
	...
	if (s_militaryPop < 5) then	
		aitrace("DetermineDemandWithNoCounterInfo: sg_randFavorShipType:" .. sg_randFavorShipType)
		if (s_race == Race_Hiigaran) then	
			if (sg_randFavorShipType < 55) then	
				ShipDemandAddByClass(eFighter, FiDemand + 1)
			elseif (sg_randFavorShipType < 85) then	
				ShipDemandAddByClass(eCorvette, CoDemand + 1)
			elseif (g_LOD < 2 and sg_randFavorShipType < 95) then	
				ShipDemandAddByClass(eFrigate, FrDemand + 1)
			else
		...
		aitrace("DetermineDemandWithNoCounterInfo: Asking for AntiFrigates")
...
function DetermineCounterDemand()	
	...
	if (s_enemyIndex ~=  - 1) then	
		...
		if (enemyMilitaryCount >= 3) then	
			-- could increase this number (should have a few ships before counters start calculating)
...
function DetermineSpecialDemand()	
	...
	-- DEFINED ROLES stuff
	if (DefinedRolesActivated == 1) then
		numSpecial = PlayersUnitTypeCount(player_enemy, player_total, eCapture) + PlayersUnitTypeCount(player_enemy, player_total, eShield)
		numFrigates = PlayersUnitTypeCount(player_enemy, player_total, eFrigate)
	end
	...
	if (s_race == Race_Hiigaran) then	
		...
		if (IsResearchDone(IMPROVEDTORPEDO) == 1 or s_militaryStrength > 40 or numRUs > 2500) then	
			torpedoDemand = 0
		end
		ShipDemandAdd(HGN_TORPEDOFRIGATE, torpedoDemand)
	end
	-- always add some demand for these two ships - if we are economically sound
	if ((GetNumCollecting() > 9 * s_numAlliesToCount or numRUs > 1500) and s_militaryPop > 8 and UnderAttackThreat() <  - 75) then	
...
function CpuBuild_DefaultShipDemandRules_Easy()	
	...
	aitrace("CpuBuild_DefaultShipDemandRules_Easy: minMilitaryValue:" .. minMilitaryValue .. " s_selfTotalValue:" .. s_selfTotalValue .. " s_enemyTotalValue:" .. s_enemyTotalValue)
...
function CpuBuild_DefaultShipDemandRules_Med()	
	...
	aitrace("CpuBuild_DefaultShipDemandRules_Med: minMilitaryValue:" .. minMilitaryValue .. " s_selfTotalValue:" .. s_selfTotalValue .. " s_enemyTotalValue:" .. s_enemyTotalValue)
	-- if we are lower then our goal then do the counter value stuff
	if (s_selfTotalValue < minMilitaryValue) then	
		DetermineClassDemand()
		DetermineCounterDemand()
	...
	if (sg_militaryDemand > 0) then	
		if (DoMilitaryBuild() == 1) then	
			...
			if (sg_resourceDemand <= 0) then	
				...
				--aitrace("**DoMilitaryBuild: milbuild")
			end
			return
			1
		end
...
function DoMilitaryBuild()
	...
	aitrace("**DoMilitaryBuild: sg_shipDemand:"..sg_shipDemand.." ssDemand:"..ssDemand)
	...
	if (UnderAttackThreat() >  - 5 and shipId > 0 and highestPriority >= 0.5) then	
		...
		ssDemand = 0
		aitrace("**DoMilitaryBuild: Subsys demand reduced because we are under attack")
	end
	if (sg_shipDemand > ssDemand) then	
		aitrace("**DoMilitaryBuild: shipId:"..shipId.." highestPriority:"..highestPriority )
		if (shipId > 0) then	
			-- build the selected military ship
			--aitrace("**DoMilitaryBuild: Attempt")
			Build(shipId)
...
function DetermineScoutDemand()	
	...
	if (UnderAttackThreat() <  - 10 and s_militaryPop > 0 and curGameTime > sg_randScoutStartBuildTime) then	
		...
		aitrace("DetermineScoutDemand: numScoutsDemanded:"..numScoutsDemanded.." shipCount:"..shipCount )
...
function DetermineBuilderClassDemand()	
	...
	if (g_LOD == 0 and numBuilders > 3 * s_numAlliesToCount) then	
		ShipDemandAddByClass(eBuilder, - 10)
		return
	elseif (g_LOD == 1 and numBuilders > 4 * s_numAlliesToCount) then	
		ShipDemandAddByClass(eBuilder, - 10)
		return
	end
	...
	local numRUs = GetRU()
	if (GetNumCollecting() < 7 * s_numAlliesToCount and numRUs < 15000) then	
		return
	end
	--aitrace("DetermineBuilderClassDemand: Builder Demand")
	...
	--aitrace("DetermineBuilderClassDemand: dif:"..dif)
	if (dif > 0) then	
		...
		--aitrace("DetermineBuilderClassDemand: bonusDemand:"..bonusDemand)
	end
	...
	if (dif <  - 2000) then	
		...
		--aitrace("DetermineBuilderClassDemand: penaltyDemand:"..penaltyDemand)
	end
	...
	--~ 		--aitrace("DetermineBuilderClassDemand: threatPenalty:"..threatPenalty)
	--~ 	end
	...
	--aitrace("DetermineBuilderClassDemand: neededMilitaryValue:"..neededMilitaryValue.." s_selfTotalValue:"..s_selfTotalValue)
	...
	--aitrace("DetermineBuilderClassDemand: militaryDifDemand:"..militaryDifDemand)
