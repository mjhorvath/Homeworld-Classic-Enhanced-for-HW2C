...
function DoResearchTechDemand_Vaygr()	
	...
	if (Util_CheckResearch(BOMBERIMPROVEDBOMBS)) then	
		...
		if (DefinedRolesActivated == 1) then
-- issue: function usually takes a "e<Thing>" type variable. May not work. Maybe duplicate what exists for PLASMABOMBS.
			numBombers = PlayersUnitTypeCount(player_ally, player_total, eVGR_BOMBER)
		end
...
function DoResearchTechDemand_Hiigaran()	
	...
	if (Util_CheckResearch(ATTACKBOMBERIMPROVEDBOMBS)) then	
		...
		if (DefinedRolesActivated == 1) then
-- issue: function usually takes a "e<Thing>" type variable. May not work. Maybe duplicate what exists for PLASMABOMBS.
			numBombers = PlayersUnitTypeCount(player_ally, player_total, eHGN_ATTACKBOMBER)
		end
		if (numBombers > 2) then	
	...
	if (Util_CheckResearch(IMPROVEDTORPEDO)) then	
		...
		if (DefinedRolesActivated == 1) then
-- issue: function usually takes a "e<Thing>" type variable. May not work. Maybe duplicate what exists for PLASMABOMBS.
			numTorpedoFrigs = PlayersUnitTypeCount(player_ally, player_total, eHGN_TORPEDOFRIGATE)
		end
...
function DoUpgradeDemand_Hiigaran()	
	...
	if (s_militaryStrength > 10 or g_LOD == 0) then	
		...
		if (DefinedRolesActivated == 1) then
			numPlatforms = PlayersUnitTypeCount(player_ally, player_total, ePlatform)
		end
		if (numPlatforms > 1) then	
			local numGunTurret = NumSquadrons(HGN_GUNTURRET)
			if (DefinedRolesActivated == 1) then
-- issue: function usually takes a "e<Thing>" type variable. May not work. Maybe duplicate what exists for PLASMABOMBS.
				numGunTurret = PlayersUnitTypeCount(player_ally, player_total, eHGN_GUNTURRET)
			end
			...
			if (DefinedRolesActivated == 1) then
-- issue: function usually takes a "e<Thing>" type variable. May not work. Maybe duplicate what exists for PLASMABOMBS.
				numIonTurret = PlayersUnitTypeCount(player_ally, player_total, eHGN_IONTURRET)
			end
		...
		if (DefinedRolesActivated == 1) then
-- issue: function usually takes a "e<Thing>" type variable. May not work. Maybe duplicate what exists for PLASMABOMBS.
-- issue: maybe switch to kCollector instead of eCollector if it works.
		...
		if (DefinedRolesActivated == 1) then
-- issue: function usually takes a "e<Thing>" type variable. May not work. Maybe duplicate what exists for PLASMABOMBS.
-- issue: maybe switch to kRefinery instead of eRefinery if it works.
			numRefinery = PlayersUnitTypeCount(player_ally, player_total, eRefinery)
		end
...
-- issue: don't do defined roles stuff here since only "capital" roles build these ships anyway
...
-- issue: don't do defined roles stuff here since only "capital" roles build these ships anyway
	...
	if (DefinedRolesActivated == 1) then
		numFighter = PlayersUnitTypeCount(player_ally, player_total, eFighter)
	end
	if (numFighter > 1) then	
		...
		if (DefinedRolesActivated == 1) then
-- issue: function usually takes a "e<Thing>" type variable. May not work. Maybe duplicate what exists for PLASMABOMBS.
			numInterceptors = PlayersUnitTypeCount(player_ally, player_total, eHGN_INTERCEPTOR)
		end
...
-- issue: don't do defined roles stuff here since only "capital" roles build these ships anyway
	...
	if (DefinedRolesActivated == 1) then
		numCorvette = PlayersUnitTypeCount(player_ally, player_total, eCorvette)
	end
	if (numCorvette > 1) then	
		local numAssaultCorvette = NumSquadrons(HGN_ASSAULTCORVETTE)
		if (DefinedRolesActivated == 1) then
-- issue: function usually takes a "e<Thing>" type variable. May not work. Maybe duplicate what exists for PLASMABOMBS.
			numAssaultCorvette = PlayersUnitTypeCount(player_ally, player_total, eHGN_ASSAULTCORVETTE)
		end
		...
		if (DefinedRolesActivated == 1) then
-- issue: function usually takes a "e<Thing>" type variable. May not work. Maybe duplicate what exists for PLASMABOMBS.
			numPulsarCorvette = PlayersUnitTypeCount(player_ally, player_total, eHGN_PULSARCORVETTE)
		end
	...
	if (DefinedRolesActivated == 1) then
		numFrigate = PlayersUnitTypeCount(player_ally, player_total, eFrigate)
	end
	if (numFrigate > 2) then	
		local numTorpedoFrigate = NumSquadrons(HGN_TORPEDOFRIGATE)
		if (DefinedRolesActivated == 1) then
-- issue: function usually takes a "e<Thing>" type variable. May not work. Maybe duplicate what exists for PLASMABOMBS.
			numTorpedoFrigate = PlayersUnitTypeCount(player_ally, player_total, eHGN_TORPEDOFRIGATE)
		end
		...
		if (DefinedRolesActivated == 1) then
-- issue: function usually takes a "e<Thing>" type variable. May not work. Maybe duplicate what exists for PLASMABOMBS.
			numIonFrigate = PlayersUnitTypeCount(player_ally, player_total, eHGN_IONCANNONFRIGATE)
		end
		...
		if (DefinedRolesActivated == 1) then
-- issue: function usually takes a "e<Thing>" type variable. May not work. Maybe duplicate what exists for PLASMABOMBS.
			numAssaultFrigate = PlayersUnitTypeCount(player_ally, player_total, eHGN_ASSAULTFRIGATE)
		end
...
-- issue: don't do it here since only "capital" roles build these ships anyway
