...
function SpecialFleets_Init()
	for playerIndex = 0, (Universe_PlayerCount() - 1) do
		local PlayerFleetName = GetGameSettingAsString("PlayerFleet" .. playerIndex)
		local sFleet = {}
		...
		for j = 1, hold_amt do
			...
			for k = 1, getn(shipsList) do
				...
				for l = 1, getn(kCount.items) do
					...
					if (strlower(lCount.type) == strlower(sShip)) then
						sName = lCount.name
						isMatch = 1
						break
					end
		...
		for j = 1, hypr_amt do
			...
			for k = 1, getn(shipsList) do
				...
				for l = 1, getn(kCount.items) do
					local lCount = kCount.items[l]
					if (strlower(lCount.type) == strlower(sShipH)) then
						sNameH = lCount.name
						isMatch = 1
						break
					end
		...
		for j = 1, subs_amt do
			...
			for k = 1, getn(subsystemsList) do
				...
				for l = 1, getn(kCount.items) do
					local lCount = kCount.items[l]
					if (strlower(lCount.type) == strlower(sSubsys)) then
						sName = lCount.name
						isMatch = 1
						break
					end
		...
		for j = 1, resc_amt do
			...
			for k = 1, getn(researchList) do
				...
				for l = 1, getn(kCount.items) do
					local lCount = kCount.items[l]
					if (strlower(lCount.type) == strlower(sResearch)) then
						sName = lCount.name
						isMatch = 1
						break
					end
